...
(
  `id` int primary key comment '自动增长编号',
...
create table `ge_enterprise_scale`代码cmd
(
  `id`  int primary key auto_increment comment '自动增长编号',
...
CREATE TABLE `ge_institution` (
  ...
  `name` varchar(64) DEFAULT NULL COMMENT '事业单位名称',
...
create table ge_share(
  ...
  policy_type varchar(16) comment '政策对象类型(政策，申报政策，政策解读)',
...
create table ge_read(
  ...
  policy_type varchar(16) comment '政策对象类型(政策，申报政策，政策解读，公告)',
  operate_time datetime comment '操作时间'
)comment = '阅读表';
...
create table `ge_user`(
  ...
  `number` varchar(32) unique comment '账号',					--唯一约束
  ...
  `status` varchar(16) comment '状态，登录、未激活、禁用,未登录',
...
CREATE TABLE `ge_role` (
  ...
  `nickname` varchar(32) comment '昵称（中文名）',
  PRIMARY KEY (`id`)
)COMMENT = '角色表';
...
CREATE TABLE `ge_permission` (
  ...
  `menu` varchar(32) comment '菜单',
  `wildcard` varchar(32) comment '权限通配符',
  `explain` varchar(32) comment '通配符释义',
  PRIMARY KEY (`id`)
)COMMENT = '权限表';
...
-- 角色权限关联表
CREATE TABLE `ge_role_permission_relation` (
